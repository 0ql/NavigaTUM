...
def scrape_rooms():
    ...
    with ThreadPool() as pool:
        for room in tqdm.tqdm(
            pool.imap_unordered(_download_and_merge_room, base_info),
            desc="Downloaded nat rooms",
            total=len(base_info),
        ):
...
def _get_base_room_infos():
    ...
    """
    cache_name = ROOMS_CACHE_DIR / "base_info.json"

    total_hits = _cached_json(cache_name)
    if total_hits is not None:
        return total_hits

    prog = tqdm.tqdm(desc="Downloading nat base room info", total=50_000)
...
def _try_download_room_base_info(start, batch):
    dl = _download_file(
        f"{NAT_API_URL}/?limit={batch}&offset={start}",
        ROOMS_CACHE_DIR / f"rooms_base_{start}_to_{(start + 1) * batch - 1}.json",
        quiet=True,
    )
...
def _report_undownloadable(undownloadable: list[int]):
    ...
    print(f"Warning: The following spans could not be downloaded:")
    ...
    for i in range(1, len(undownloadable)):
        if undownloadable[i] == end + 1:
            ...
            if start == end:
                ...
                print(f"\t{start}->{end} ({end - start + 1} rooms)")
...
def _join_room_hits():
    ...
    """
    total_hits = []
    for file_path in ROOMS_CACHE_DIR.iterdir():
